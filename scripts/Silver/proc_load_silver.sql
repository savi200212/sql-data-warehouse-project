...
END AS sls_due_dt,
CASE 
      WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_qauntity * ABS(sls_price) 
...
CASE 
	 ...
	 THEN sls_sales / NULLIF(sls_qauntity,0)
...
INSERT INTO silver.erp_loc_a101(cid,centry)
SELECT
REPLACE(cid, '-', '') cid,
CASE WHEN TRIM(centry) = 'DE' THEN 'Germany'
	 ...
	 ELSE TRIM(centry) 
END AS centry ---Normalize and Handle missing or blank country codes
